...
{
    /// <summary>
    /// Customized from Dapper.SimpleCRUD
    /// </summary>
    public static partial class ModactCRUD
    {
        ...
        public static string Insert<TEntity>(this IDbConnection connection, TEntity entityToInsert, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            //Modact customized (int to string, default use varchar key)
        ...
        public static TKey Insert<TKey, TEntity>(this IDbConnection connection, TEntity entityToInsert, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            ...
            //Modact customized, use varchar Ulid as key
            if (keytype == typeof(string))
            {
                var stringvalue = (string)idProps.First().GetValue(entityToInsert, null);
                if (string.IsNullOrEmpty(stringvalue))
                {
                    var newulid = Modact.Ulid.NewUlid().ToString();
                    idProps.First().SetValue(entityToInsert, newulid, null);
                }
                else
                {
        ...
        public static int UpdateWithoutNull<TEntity>(this IDbConnection connection, TEntity entityToUpdate, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            if (typeof(TEntity).IsInterface) //FallBack to BaseType Generic Method: https://stackoverflow.com/questions/4101784/calling-a-generic-method-with-a-dynamic-type
            {
                return (int)typeof(ModactCRUD)
            ...
            var masterSb = new StringBuilder();
            StringBuilderCache(masterSb, $"{typeof(TEntity).FullName}_UpdateWithoutNull", sb =>
            {
                var idProps = GetIdProperties(entityToUpdate).ToList();
                ...
                sb.AppendFormat(" set ");
                BuildUpdateSetWithoutNull(entityToUpdate, sb);
                sb.Append(" where ");
                BuildWhere<TEntity>(sb, idProps, entityToUpdate);
        ...
        //build update statement based on list on an entity
        private static void BuildUpdateSetWithoutNull<T>(T entityToUpdate, StringBuilder masterSb)
        {
            StringBuilderCache(masterSb, $"{typeof(T).FullName}_BuildUpdateSetWithoutNull", sb =>
            {
                var nonIdPropsList = GetUpdateableProperties(entityToUpdate).ToArray();

                var nonIdProps = nonIdPropsList.Where(p => p.GetValue(entityToUpdate) != null).ToArray();
            ...
            for (var i = 0; i < propertyInfos.Count(); i++)
            {
                ...
                for (var x = 0; x < sourceProperties.Count(); x++)
                {
                    if (sourceProperties.ElementAt(x).Name == propertyToUse.Name)
                    {
                        ...
                        //propertyToUse = sourceProperties.ElementAt(x); //Modact customized, for check is array or not
                        break;
                    }
                }
                if (propertyToUse.PropertyType.IsArray || (propertyToUse.PropertyType.GetTypeInfo().IsGenericType && propertyToUse.PropertyType.GetGenericTypeDefinition() == typeof(List<>))) //Modact customized, check is array or not
                {
                    sb.AppendFormat("{0} in @{1}", GetColumnName(propertyToUse), propertyToUse.Name);
                }
                else
                {
                    sb.AppendFormat(
            ...
            StringBuilderCache(masterSb, $"{typeof(T).FullName}_BuildInsertValues", sb =>
            {
                ...
                for (var i = 0; i < props.Count(); i++)
                {
                    ...
                    if (property.Name.Equals("Id", StringComparison.OrdinalIgnoreCase) 
                          && property.GetCustomAttributes(true).All(attr => attr.GetType().Name != typeof(RequiredAttribute).Name)
                          && property.PropertyType != typeof(string) //String(char/nchar/varchar/nvarchar) type ID need to insert
